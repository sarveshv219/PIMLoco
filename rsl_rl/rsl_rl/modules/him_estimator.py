...
class HIMEstimator(nn.Module):
    def __init__(self,
        ...
        # Encoder
        enc_input_dim = self.temporal_steps * self.num_one_step_obs + self.num_scandots
        # enc_input_dim = self.temporal_steps * self.num_one_step_obs + 117
        print(self.temporal_steps)
        enc_layers = []
        ...
        # Target
        tar_input_dim = self.num_one_step_obs + self.num_scandots
        # tar_input_dim = self.num_one_step_obs + 117
        tar_layers = []
    ...
    def forward(self, obs_history, scandots):
        parts = self.encoder(torch.cat((obs_history.detach(), scandots.detach()), dim=-1))
    ...
    def update(self, obs_history, scandots, next_critic_obs, lr=None):
        if lr is not None:
            self.learning_rate = lr
            for param_group in self.optimizer.param_groups:
                param_group['lr'] = self.learning_rate
        
        print("Obs History shape:", obs_history.shape)
        print("Next Critic Input shape:", next_critic_obs.shape)   
        ...
        next_scandots = next_critic_obs.detach()[:, -117:]
        # next_obs = torch.cat((next_critic_obs.detach()[:, 3:self.num_one_step_obs+3], 
